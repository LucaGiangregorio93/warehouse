...
cap program drop eigt_verify
	...
	//////////////////////////////////////////////
	 display "2. Inadmissible entries in mandatory variables..."
		...
		destring different_tax, replace
	...
	restore 
		...
		forvalues i = 1(1)`nbr' {
			...
			local j = `i' - 1
				if `i' != `nbr' {	
					replace schedulelowerbounds`i' = schedulelowerbounds`k' if round(schedulelowerbounds`i', .01) == round(schedulelowerbounds`j', .01) & schedulelowerbounds`i' != 1 & schedulelowerbounds`i' != 1 + exemption
