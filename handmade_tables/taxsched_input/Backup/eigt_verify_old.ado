...
cap program drop eigt_verify
	...
	// Check if the destring was successful 
			...
			forvalues i = 1/`nmr' {				
				local j = `i' + 1
				if (`i' != `nmr') {
					qui count if marginalrates`i' == marginalrates`j' & (marginalrates`i' != . & marginalrates`i' != 0)
					if (`r(N)' != 0) {
						display as error "WARNING: marginal rate = to the preceding one, the brackets will be unified"
						tab marginalrates`i' if marginalrates`i' == marginalrates`j' & ( marginalrates`i' != .  & marginalrates`i' != 0)
						}
					}					
				}

	// Address cases with multiple zeros at the beginning of the rates (e.g. Italy)
		forvalues i = 1/`nmr' {				
			local j = `i' + 1
			if (`i' != `nmr') {
				qui count if marginalrates`i' == 0 & marginalrates`j' == 0
				if (`r(N)' != 0) {
					display as error "WARNING: multiple zeros in marginal rate, the brackets will be unified"
					tab tax year_from if marginalrates`i' == 0 & marginalrates`j' == 0
					local tounify yes
					}
				}					
			}
		if "`tounify'" == "yes" {	
			forvalues i = 1/`nmr' {
				qui replace schedulelowerbounds`i' = 0 if marginalrates`i' == 0
				qui replace scheduleupperbounds`i' = 0 if marginalrates`i' == 0					
			}	
			forvalues i = 1/`nmr' {
				local j = `i' + 1
				local w = `i' + 2
				forvalues z = `w'/`nmr' {
					qui replace marginalrates`j' = marginalrates`z' if marginalrates`j' == 0 & marginalrates`z' != 0 & marginalrates`z' > marginalrates`i' 
					qui replace schedulelowerbounds`j' = schedulelowerbounds`z' if schedulelowerbounds`j' == 0 & schedulelowerbounds`z' != 0 & schedulelowerbounds`z' > schedulelowerbounds`i' 		
					qui replace scheduleupperbounds`j' = scheduleupperbounds`z' if scheduleupperbounds`j' == 0 & scheduleupperbounds`z' != 0 & scheduleupperbounds`z' > scheduleupperbounds`i' 					
				}
			}
			local wtv = `nmr'-1
			forvalues k = 1/`nmr' {
				forvalues i = 2/`wtv' {
					local j = `i' + 1
					local y = `i' - 1
						qui replace marginalrates`i' = marginalrates`j' if marginalrates`i' <= marginalrates`y'					
						qui replace schedulelowerbounds`i' = schedulelowerbounds`j' if schedulelowerbounds`i' <= schedulelowerbounds`y'					
						qui replace scheduleupperbounds`i' = scheduleupperbounds`j' if scheduleupperbounds`i' <= scheduleupperbounds`y'							
				}
			}	
			forvalues i = `nmr'(-1)2 {
				local y = `i' - 1
				qui replace marginalrates`i' = . if marginalrates`i' == marginalrates`y'
				qui replace schedulelowerbounds`i' = . if schedulelowerbounds`i' == schedulelowerbounds`y'
				qui replace scheduleupperbounds`i' = . if scheduleupperbounds`i' == scheduleupperbounds`y'  
			}
			qui replace scheduleupperbounds1 = -997 if scheduleupperbounds1 == 0 & scheduleupperbounds2 == . 			
			forvalues i = 2/`nmr' {
				local y = `i' - 1
				qui replace scheduleupperbounds`i' = -997 if scheduleupperbounds`i' == . & scheduleupperbounds`y' != . & scheduleupperbounds`y' != -997
			}			
		}
		drop marginalrates			
